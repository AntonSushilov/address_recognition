...
def save_to_txt(file, file_img, output_dir, text):
    name = file.split(".")[0] + '.txt'
    # name_page = file_img.split(".")[0]
    print(file_img)
    text = file_img + "+" + text.strip().replace("  ", " ")
...
def image_to_text(img_dir, output_dir, file):
    ...
    for file_img in files:
        image = os.path.join(img_dir, file_img)
        ...
        save_to_txt(file, file_img, output_dir, text)
